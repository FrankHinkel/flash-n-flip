...
class Parser:
    ...
    def doNotations (s, note, nttn, isTab):
        ...
        for finger in fingering:    # handle multiple finger annotations
            # Flash-n-Flip patch: numeric decorations such as !3! are not
            # consistently rendered by abcjs. Emit standard ABC text
            # annotations and retain MusicXML placement instead.
            raw_finger = (finger.text or '').strip ()
            if not isTab and re.fullmatch (r'[1-5](?:\s*[-–]\s*[1-5])?', raw_finger):
                raw_finger = re.sub (r'\s*[-–]\s*', '-', raw_finger)
                if finger.get ('alternate') == 'yes': raw_finger = '(%s)' % raw_finger
                placement = finger.get ('placement')
                if placement not in ('above', 'below'):
                    placement = 'above' if note.xmlstf == 1 else 'below'
                note.before += ['"%s%s"' % ('^' if placement == 'above' else '_', raw_finger)]
            elif not isTab and raw_finger:
                info ('unsupported fingering %s discarded' % raw_finger)
    ...
    def doDirection (s, e, i, es): # parse a musicXML direction tag
        ...
        if tempo:
            # Flash-n-Flip patch: malformed optional tempo values must not
            # abort conversion of an otherwise valid score.
            try: tempo = '%.0f' % float (tempo)
            except (TypeError, ValueError):
                info ('invalid tempo %s discarded' % tempo)
                tempo = ''
        if tempo:
