...
describe("browser cloud binding persistence", () => {
  it("retains pending and confirmed bindings across database reopen", async () => {
    ...
    expect(
      await createBrowserCloudLibraryBindings(name).read("development"),
    ).toEqual({ ...candidate(), phase: "bound" });
  });

  it("serializes racing tabs and refuses a second account", async () => {
    const name = `binding-${crypto.randomUUID()}`;
    const a = createBrowserCloudLibraryBindings(name),
      b = createBrowserCloudLibraryBindings(name);
    const results = await Promise.allSettled([
      a.reserve(candidate()),
      b.reserve(candidate("account-b")),
    ]);
    expect(
  ...
  it("refuses confirmation without the exact pending binding", async () => {
    ...
    await expect(
      bindings.confirm(candidate(), candidate().root),
    ).rejects.toMatchObject({ code: "LOCAL_BINDING_CHANGED" });
    expect(await bindings.read("development")).toBeNull();
    await bindings.reserve(candidate());
    await expect(
      bindings.confirm(candidate("account-b"), candidate().root),
    ).rejects.toMatchObject({ code: "ACCOUNT_MISMATCH" });
    expect(await bindings.read("development")).toEqual(candidate());
  });

  it("persists an explicitly validated development generation adoption", async () => {
    const bindings = createBrowserCloudLibraryBindings(
      `binding-${crypto.randomUUID()}`,
    );
    const pending = await bindings.reserve(candidate());
    await bindings.confirm(pending, pending.root);
    const bound = (await bindings.read("development"))!;
    const replacement = {
      ...bound.root,
      libraryGeneration: "20000000-0000-4000-8000-000000000002",
    };
