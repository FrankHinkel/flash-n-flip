...
function fixture(records: Record<string, { value: unknown; changeTag: string }> = {}) {
  const values = new Map(Object.entries(records));
  const compareAndSwap = vi.fn(async (name: string, expectedTag: string | null, value: unknown) => {
    const previous = values.get(name);
    if ((previous?.changeTag ?? null) !== expectedTag) throw new Error("conflict");
    values.set(name, { value, changeTag: `tag-${compareAndSwap.mock.calls.length}` });
  });
  ...
  const atomicStoreForIdentity = vi.fn((identity: typeof oldIdentity) => ({
    createZone,
    deleteZone,
    read: vi.fn(async (name: string) => name === atomicCloudRootName
      ? zones.get(identity.libraryGeneration) ?? null
      : null),
    atomic: vi.fn(async () => undefined),
  }));
  const initialize = vi.fn(async (_store: unknown, identity: typeof oldIdentity) => {
    zones.set(identity.libraryGeneration, { value: atomicRoot(identity), changeTag: "atomic-tag" });
  });
...
describe("development cloud reset", () => {
  it("rebuilds an externally reset empty development library", async () => {
    ...
    expect(f.values.get(cloudLibraryRootRecordName)?.value).toEqual(root(nextIdentity));
  ...
  it("deletes an intact old zone before replacing its generation", async () => {
    const f = fixture({
      [cloudLibraryRootRecordName]: { value: root(oldIdentity), changeTag: "root-old" },
      [cloudLibraryZoneMarkerRecordName]: { value: { ...oldIdentity, phase: "ready" }, changeTag: "marker-old" },
    });
  ...
  it("lets a stale second device adopt the already confirmed empty generation", async () => {
    const f = fixture({
      [cloudLibraryRootRecordName]: { value: root(nextIdentity), changeTag: "root-next" },
      [cloudLibraryZoneMarkerRecordName]: { value: { ...nextIdentity, phase: "ready" }, changeTag: "marker-next" },
    });
    f.zones.set(nextIdentity.libraryGeneration, { value: atomicRoot(nextIdentity), changeTag: "atomic-tag" });
    await expect(replaceDevelopmentCloudLibrary(f.input)).resolves.toEqual({
      identity: nextIdentity,
      mode: "adopted",
    });
  ...
  it("does not accept a remote generation without its confirmed custom-zone root", async () => {
    ...
    await expect(replaceDevelopmentCloudLibrary(f.input)).rejects.toThrow(/zone is missing/);
    expect(f.compareAndSwap).not.toHaveBeenCalled();
  });

  it("refuses to modify production", async () => {
    const f = fixture();
    await expect(replaceDevelopmentCloudLibrary({ ...f.input, environment: "production" }))
