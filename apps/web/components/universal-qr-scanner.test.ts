...
describe("universal QR scanner UI", () => {
  it("provides one global camera action and centralizes incoming share links", () => {
    expect(appShell).toContain("<UniversalQrScanner />");
    expect(component).toContain('aria-label={text("Scan QR code"');
  ...
  it("keeps keyboard, paste, and small-screen alternatives available", () => {
    ...
    expect(styles).toMatch(
      /\.universal-qr-trigger\s*\{[^}]*left:\s*50%;[^}]*translateX\(-50%\)/s,
    );
    expect(styles).toMatch(
