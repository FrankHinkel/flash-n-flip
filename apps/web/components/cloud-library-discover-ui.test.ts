...
const cloud = readFileSync(
  new URL("./cloud-deck-browser.tsx", import.meta.url),
  "utf8",
);
const settings = readFileSync(
  new URL("./cloud-library-sync-setting.tsx", import.meta.url),
  "utf8",
);
const runtime = readFileSync(
  new URL("../lib/cloud-library-runtime.ts", import.meta.url),
  "utf8",
);
...
describe("Discover iCloud deck management", () => {
  it("keeps Apple controls inside both cloud management layouts", () => {
    ...
    expect(styles).not.toMatch(
      /\.cloud-account-actions\s*\{[^}]*position:\s*fixed/s,
    );
  });
  ...
  it("keeps a textual account state and 44px tab/action targets", () => {
    ...
    expect(styles).toMatch(
      /\.discover-source-tabs button\s*\{[^}]*min-height:\s*44px/s,
    );
    expect(styles).toMatch(
      /\.cloud-deck-actions \.button\s*\{[^}]*min-height:\s*44px/s,
    );
  });
  it("lets the development reset stop a running synchronization before deletion", () => {
    expect(settings).toContain(
      "disabled={view.stopping || resetting || !view.account}",
    );
    ...
    expect(runtime).toMatch(
      /async function resetDevelopmentFlashNFlipData[\s\S]*?if \(inFlight \|\| pausing\) await pauseCloudSync\(\);/,
    );
  });
  it("publishes discovered decks incrementally and does not repeat every completed full scan", () => {
    ...
    expect(runtime).toContain("upsertCloudDeckResult(view.decks, deck)");
