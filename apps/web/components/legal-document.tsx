  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          [
            "After sign-in, users can explicitly connect the library. This stores a random library identifier and generation with protocol/deletion metadata in the private CloudKit database, and an account binding in this browser's IndexedDB. The binding survives logout and prevents silent attachment to a different account. No decks, media or learning progress are transferred in this step. Removing browser-site data removes the local binding; cloud identifiers remain in the private container until explicitly deleted there. In-app cloud deletion is not implemented yet.",
            "Nach der Anmeldung kann die Bibliothek ausdrücklich verbunden werden. Dabei werden eine zufällige Bibliothekskennung und Generation mit Protokoll-/Löschmetadaten in der privaten CloudKit-Datenbank sowie eine Kontobindung in IndexedDB dieses Browsers gespeichert. Die Bindung bleibt beim Abmelden erhalten und verhindert eine unbemerkte Zuordnung zu einem anderen Konto. Decks, Medien und Lernfortschritte werden dabei nicht übertragen. Das Entfernen der Website-Daten löscht die lokale Bindung; die Cloud-Kennungen bleiben bis zur ausdrücklichen Löschung im privaten Container erhalten. Die Cloud-Löschung innerhalb der App ist noch nicht implementiert.",
          ],
          [
            "Configured Web/PWA builds offer optional iCloud authentication. Only starting it loads CloudKit JS and contacts Apple, which receives technical connection data and manages authentication. CloudKit JS persists the session in the browser; users can sign out using Apple's control. The app uses the container-specific account identifier to display authentication status, without uploading decks, media or learning progress. Signing out does not delete local or cloud learning data. Native builds can include the CloudKit entitlement, but library synchronization, encrypted backup and family sharing remain inactive. Apple's processing terms and the legal basis for this optional flow require review before public activation; App Store privacy information must also be finalized.",
            "Konfigurierte Web-/PWA-Builds bieten eine optionale iCloud-Anmeldung. Erst deren Start lädt CloudKit JS und kontaktiert Apple; Apple erhält technische Verbindungsdaten und verwaltet die Anmeldung. CloudKit JS speichert die Sitzung im Browser; die Abmeldung erfolgt über Apples Bedienelement. Die App verwendet die containerspezifische Account-Kennung zur Statusanzeige, ohne Decks, Medien oder Lernfortschritte hochzuladen. Abmelden löscht keine lokalen oder Cloud-Lerndaten. Native Builds können die CloudKit-Berechtigung enthalten; Bibliothekssynchronisierung, verschlüsseltes Backup und Familienfreigabe bleiben jedoch inaktiv. Apples Verarbeitungsbedingungen und die Rechtsgrundlage dieses optionalen Ablaufs müssen vor öffentlicher Aktivierung geprüft und die App-Store-Datenschutzangaben vervollständigt werden.",
          ],
        ],
      },
      {
