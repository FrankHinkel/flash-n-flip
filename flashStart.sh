...
info() {
  printf '\033[1;36m[FlashCards]\033[0m %s\n' "$1"
}

success() {
  printf '\033[1;32m[FlashCards]\033[0m %s\n' "$1"
}

fail() {
  printf '\033[1;31m[FlashCards]\033[0m %s\n' "$1" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Lokale FlashCards-Entwicklungsumgebung starten.
...
cleanup() {
  ...
  if [[ "$exit_code" -eq 0 ]]; then
    ...
    printf '\n\033[1;31m[FlashCards]\033[0m Entwicklungsumgebung wurde mit Status %s beendet.\n' \
      "$exit_code" >&2
    exit "$exit_code"
  fi
}
...
FlashCards startet jetzt im lokalen Entwicklungsmodus:
