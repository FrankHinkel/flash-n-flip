...
import {
  ...
  CloudLibraryError,
  publishCloudReview,
  type CloudRecordStore,
} from "@flashcards/sync/cloud-library";
import {
  canonicalLocalAuthorityPayloadBytes,
  maximumLocalMutationBatchSize,
  type LocalAuthorityRepository,
} from "@flashcards/sync/local-authority";
import {
  cloudCardContent,
  CloudContentConflict,
  parseCloudDeckContent,
  planCloudDeckProjection,
  type CloudDeckContent,
} from "@flashcards/sync/cloud-library-projection";
import {
  cloudAssetChunkBytes,
  stageCloudAsset,
  uploadCloudAssets,
  verifyAssembledCloudAsset,
  type CloudAssetCodec,
} from "@flashcards/sync/cloud-library-assets";
import {
  createCloudAssetStaging,
  type CloudDurableKeyValue,
...
const same = (a: unknown, b: unknown) => {
  const left = canonicalLocalAuthorityPayloadBytes(a),
    right = canonicalLocalAuthorityPayloadBytes(b);
  return (
...
export type CloudDeckSyncState = {
  ...
  pendingReviews?: CloudReviewEvent[];
...
type ContentPackage = {
  format: "flash-n-flip.cloud-content.v2";
  content: CloudDeckContent;
  media: MediaAsset[];
};
...
export type CloudDeckSyncResult = {
  ...
  title: string;
  removed: boolean;
  status: "pending" | "synced" | "conflict" | "error" | "deleted";
  revisions: string[];
  problem?: CloudTransferProblem;
  cardCount: number;
...
export function prioritizeCloudDecks<T>(
  items: readonly T[],
  ...
  const ranked = items
    ...
    .sort(
      (left, right) =>
        Number(priority(right.item)) - Number(priority(left.item)) ||
...
export function mergeCloudContents(
  base: CloudDeckContent | null,
  local: CloudDeckContent | null,
  ...
  const mergeList = <T>(
    ...
    const b = new Map(before.map((v) => [id(v), v])),
      l = new Map(here.map((v) => [id(v), v])),
...
export function cloudContentFromSnapshot(
  snapshot: Snapshot,
  deckId: string,
): CloudDeckContent | null {
  const live = snapshot.payload.entities
    .filter((entity) => entity.winningMutation.operation === "UPSERT")
    .map((entity) => entity.winningMutation);
  const deck = live.find(
    (m) => m.entityId === deckId && m.entityType === "DECK",
  );
  if (!deck) return null;
  const deckPayload = localDeckPayloadSchema.parse(deck.payload);
  return parseCloudDeckContent({
    deckId,
    deck: {
      ...deckPayload,
      // Availability is device-local. Archiving or hiding a local download
      // must never archive the shared iCloud revision on another device.
      hiddenAt: null,
      archivedAt: null,
    },
    cards: live
      ...
      .flatMap((m) => {
        ...
        return card.deckId === deckId ? [{ cardId: m.entityId, card }] : [];
      })
      .sort(
        (left, right) =>
          left.card.state.due.localeCompare(right.card.state.due) ||
          left.card.position - right.card.position ||
          left.cardId.localeCompare(right.cardId),
      )
      .map(({ cardId, card }) => ({ cardId, content: cloudCardContent(card) })),
    media: live
      .filter((m) => m.entityType === "MEDIA_REFERENCE")
      .flatMap((m) => {
        const reference = localMediaReferencePayloadSchema.parse(m.payload);
        return reference.deckId === deckId
          ? [{ mediaId: m.entityId, reference }]
          : [];
      }),
  });
}
function reviewsFromSnapshot(
  ...
  return snapshot.payload.entities
    .filter((e) => e.winningMutation.entityType === "REVIEW")
    .flatMap((e) => {
      const review = localReviewPayloadSchema.parse(e.winningMutation.payload);
      return review.deckId === control.deckId
        ? [cloudReviewEventSchema.parse({ ...scope, review })]
        : [];
    });
}
...
export class CloudLibraryRuntime {
  ...
  private progress(
    stage: CloudTransferProgress["stage"],
    current = 0,
    total = 0,
    completedBytes = 0,
    totalBytes = 0,
    ...
    this.input.onProgress?.({
      ...
      overallCurrent: this.overallCurrent,
  ...
  private key(deckId: string) {
    return JSON.stringify([
      "deck-runtime-v3",
      this.input.environment,
      this.input.account,
      this.input.identity.libraryId,
      this.input.identity.libraryGeneration,
      deckId,
    ]);
  }
  async state(deckId: string): Promise<CloudDeckSyncState | null> {
    ...
    if (
      ...
      control.libraryId !== this.input.identity.libraryId ||
      control.libraryGeneration !== this.input.identity.libraryGeneration
    )
      throw new Error("Local cloud scope changed");
  ...
  private async save(state: CloudDeckSyncState) {
    await this.input.values.update(this.key(state.control.deckId), () =>
      JSON.stringify(state),
    );
  }
  private staging(manifest: CloudAssetManifest) {
    return createCloudAssetStaging({
      ...this.input,
      manifest,
  ...
  private async stageLocal(bytes: Uint8Array): Promise<CloudAssetManifest> {
    ...
    if (!bytes.length || bytes.length > maxAssetBytes)
      throw new Error("Cloud asset exceeds the 128 MiB transfer limit");
    const chunks = [];
    for (
      let offset = 0;
      offset < bytes.length;
      offset += cloudAssetChunkBytes
    ) {
      this.input.checkActive?.();
      ...
      chunks.push({
        ...
        sha256: await cloudCodec.hash(chunk),
      });
      this.progress(
        "prepare",
        0,
        1,
        Math.min(offset + chunk.length, bytes.length),
        bytes.length,
      );
    }
    const manifest = cloudAssetManifestSchema.parse({
      sha256: await cloudCodec.hash(bytes),
      byteSize: bytes.length,
      chunks,
    });
    const staging = this.staging(manifest);
    for (const chunk of chunks)
      await staging.writeChunk(
        chunk.index,
        bytes.slice(
          chunk.index * cloudAssetChunkBytes,
          chunk.index * cloudAssetChunkBytes + chunk.byteSize,
        ),
      );
    return manifest;
  }
  private async uploadPending(
    ...
    pending: NonNullable<CloudDeckSyncState["pending"]>,
    ...
    const source = (manifest: CloudAssetManifest) => {
      ...
      return {
        ...
        readChunk: async (index: number) => {
          ...
          if (!bytes)
            throw new Error(
              "Durable upload source missing; preserve pending publication",
            );
          return bytes;
        },
      };
    };
    // Deck/card content is transferred before optional media bytes.
    const sources = [
      source(pending.revision.content),
      ...pending.media.map((asset) => source(asset.manifest)),
    ];
    const total = sources.reduce(
      (sum, candidate) => sum + candidate.manifest.chunks.length,
      0,
    );
    const totalBytes = sources.reduce(
      (sum, candidate) => sum + candidate.manifest.byteSize,
      0,
    );
    this.progress("upload", 0, total, 0, totalBytes);
    await uploadCloudAssets({
      store,
      sources,
      knownRecordNames,
      identity: this.input.identity,
      codec: cloudCodec,
      onProgress: (done, count, doneBytes, bytes) =>
        this.progress("upload", done, count, doneBytes, bytes),
    });
  }
  private async download(
    ...
    if (manifest.byteSize > limit)
      throw new Error("Cloud download exceeds the device transfer limit");
    const staging = this.staging(manifest);
    await stageCloudAsset({
      store,
      identity: this.input.identity,
      manifest,
      codec: cloudCodec,
      staging,
      onProgress: (done, total, doneBytes, totalBytes) =>
        this.progress("download", done, total, doneBytes, totalBytes),
    });
  ...
  private async contentPackage(
    store: CloudRecordStore,
    revision: CloudDeckRevision,
  ): Promise<ContentPackage> {
    const value = JSON.parse(
      new TextDecoder("utf-8", { fatal: true }).decode(
        await this.download(store, revision.content, maxContentBytes),
      ),
    ) as ContentPackage;
    if (
      value.format !== "flash-n-flip.cloud-content.v2" ||
      !Array.isArray(value.media) ||
      value.media.length > 99_999
    )
      throw new Error("Invalid cloud content package");
    value.content = parseCloudDeckContent(value.content);
    if (value.content.deckId !== revision.deckId)
      throw new Error("Cloud content belongs to another deck");
    const ids = new Set<string>();
    for (const asset of value.media) {
      if (
        !/^[0-9a-f-]{36}$/i.test(asset.mediaId) ||
        ids.has(asset.mediaId) ||
        typeof asset.mimeType !== "string" ||
        asset.mimeType.length > 255
      )
        throw new Error("Invalid cloud media descriptor");
      ids.add(asset.mediaId);
      cloudAssetManifestSchema.parse(asset.manifest);
    }
    for (const id of referencedMedia(value.content))
      if (!ids.has(id)) throw new Error("Cloud package omits referenced media");
    for (const { mediaId, reference } of value.content.media) {
      const asset = value.media.find((m) => m.mediaId === mediaId)!;
      if (
        asset.manifest.sha256 !== reference.sha256 ||
        asset.manifest.byteSize !== reference.byteSize ||
        asset.mimeType !== reference.mimeType
      )
        throw new Error("Cloud media reference mismatch");
    }
    return value;
  }
  private async installMedia(store: CloudRecordStore, content: ContentPackage) {
    for (const asset of content.media) {
      ...
      if (
        previous?.sha256 === asset.manifest.sha256 &&
        previous.bytes.length === asset.manifest.byteSize &&
        (await cloudCodec.hash(previous.bytes)) === asset.manifest.sha256
      )
        continue;
      // Never overwrite bytes still referenced by concurrent local content.
      if (previous && previous.sha256 !== asset.manifest.sha256)
        throw new CloudContentConflict(asset.mediaId);
      const bytes = await this.download(store, asset.manifest);
      await this.input.media.put({
  ...
  private activation(
    control: CloudDeckControl,
    content: CloudDeckContent | null,
  ): CloudCuratedDeckActivation | null {
    if (
      !content?.deck.sourceTemplateKey ||
      !content.deck.sourceContentSha256 ||
      !content.deck.sourcePublishedAt
    )
      return null;
  ...
  private parseActivation(
    value: unknown,
    control: CloudDeckControl,
  ): CloudCuratedDeckActivation {
    const activation = cloudCuratedDeckActivationSchema.parse(value);
    if (
      activation.libraryId !== control.libraryId ||
      activation.libraryGeneration !== control.libraryGeneration ||
      activation.deckId !== control.deckId ||
      activation.deckGeneration !== control.deckGeneration
    ) {
  ...
  private async publishActivation(
    store: CloudRecordStore,
    activation: CloudCuratedDeckActivation,
  ) {
    const previous = await store.read("activation.v1");
    if (previous && !same(previous.value, activation))
      throw new CloudContentConflict(activation.deckId);
    if (!previous) {
      try {
        await store.compareAndSwap("activation.v1", null, activation);
      } catch (error) {
        if (!(
    ...
    if (!confirmed || !same(confirmed.value, activation))
      throw new CloudContentConflict(activation.deckId);
  }
  private async acknowledge(
    ...
    const deckIds = new Set(
      snapshot.payload.entities
        .filter((entity) => {
          const mutation = entity.winningMutation;
          return (
            mutation.entityId === control.deckId ||
            (mutation.payload &&
              typeof mutation.payload === "object" &&
              "deckId" in mutation.payload &&
              mutation.payload.deckId === control.deckId)
          );
        })
        .map((entity) => entity.winningMutation.entityId),
    );
    if (!(await store.read(receiptName)))
      throw new Error("Cloud receipt disappeared");
    await this.input.assertAccount();
    await this.input.authority.acknowledgeOutbox(
      snapshot.payload.mutationJournal
        .filter(
          (mutation) =>
            deckIds.has(mutation.entityId) &&
            ["DECK", "CARD", "MEDIA_REFERENCE", "REVIEW"].includes(
              mutation.entityType,
            ),
        )
  ...
  private async publish(
    state: CloudDeckSyncState,
    content: CloudDeckContent,
    parents: string[],
    knownPayloadNames: readonly string[] = [],
  ): Promise<CloudDeckRevision> {
    ...
    if (!state.pending) {
      ...
      for (const mediaId of [...referencedMedia(content)].sort()) {
        ...
        if (!bytes || (await cloudCodec.hash(bytes.bytes)) !== bytes.sha256)
          throw new Error("Local media missing or corrupt");
        media.push({
          mediaId,
          mimeType: bytes.mimeType,
          manifest: await this.stageLocal(bytes.bytes),
        });
      }
      const bytes = canonicalLocalAuthorityPayloadBytes({
        format: "flash-n-flip.cloud-content.v2",
        content,
        media,
      });
      if (bytes.length > maxContentBytes)
        throw new Error("Cloud deck content exceeds 32 MiB");
      const revision = cloudDeckRevisionSchema.parse({
        ...this.input.identity,
        protocolVersion: 1,
        deckId: state.control.deckId,
        deckGeneration: state.control.deckGeneration,
    ...
    if (previous && !same(previous.value, pending.revision))
    ...
    if (!confirmed || !same(confirmed.value, pending.revision))
      throw new Error("Revision publication not confirmed");
    await this.input.assertAccount();
  ...
  private async project(
    ...
    for (const { mediaId, reference } of content.media) {
      ...
      if (!media || (await cloudCodec.hash(media.bytes)) !== reference.sha256)
        throw new Error("Installed media is unverified");
      receipts.push({
        mediaId,
        sha256: media.sha256,
    ...
    const localDeck = snapshot.payload.entities.find(
      (entity) =>
        entity.winningMutation.entityId === control.deckId &&
        entity.winningMutation.entityType === "DECK" &&
        entity.winningMutation.operation === "UPSERT",
    );
    const localDeckPayload = localDeck
      ? localDeckPayloadSchema.parse(localDeck.winningMutation.payload)
      : null;
    const projectedContent = localDeckPayload
      ? parseCloudDeckContent({
          ...content,
          deck: {
            ...content.deck,
            hiddenAt: localDeckPayload.hiddenAt,
            archivedAt: localDeckPayload.archivedAt,
          },
        })
      : content;
    const mutations = planCloudDeckProjection({
      control,
      base: local,
      remote: projectedContent,
      entities: snapshot.payload.entities,
      localReviews: reviewsFromSnapshot(snapshot, control),
      remoteReviews,
      verifiedMedia: receipts,
      clock: this.clock(),
      allowContentDeletions: true,
    });
    this.input.checkActive?.();
    if (mutations.length)
      await this.input.authority.commitLocalMutations(mutations, {
  ...
  async synchronize(
  ...
  ): Promise<CloudDeckSyncResult[]> {
    const requestedResolutions = new Map<string, string>();
    for (const resolution of resolve
      ? Array.isArray(resolve)
        ? resolve
        : [resolve]
      : []) {
      const previous = requestedResolutions.get(resolution.deckId);
      if (previous && previous !== resolution.revisionId)
        throw new Error("Conflicting resolutions for one deck");
      requestedResolutions.set(resolution.deckId, resolution.revisionId);
    }
    ...
    const dirtyDecks = new Set(
      initial.payload.mutationJournal.flatMap((mutation) => {
        if (
          !outboxIds.has(mutation.mutationId) ||
          !["DECK", "CARD", "MEDIA_REFERENCE", "REVIEW"].includes(
            mutation.entityType,
          )
        )
    ...
    const localDecks = initial.payload.entities.filter(
      (entity) =>
        entity.winningMutation.entityType === "DECK" &&
        entity.winningMutation.operation === "UPSERT",
    );
    const localContent = new Map(
      localDecks.map((entity) => [
        entity.winningMutation.entityId,
        cloudContentFromSnapshot(initial, entity.winningMutation.entityId)!,
      ]),
    );
    const registrationOrder = parentFirst(
      localDecks,
      (entity) => entity.winningMutation.entityId,
      (entity) =>
        localDeckPayloadSchema.parse(entity.winningMutation.payload)
          .parentDeckId,
    );
    for (const entity of registrationOrder) {
      const mutation = entity.winningMutation;
      if (catalog.some((entry) => entry.control.deckId === mutation.entityId))
        continue;
      ...
      this.progress("catalog");
      const control = cloudDeckControlSchema.parse({
        ...this.input.identity,
        protocolVersion: 1,
        deckId: mutation.entityId,
        deckGeneration: crypto.randomUUID(),
        progressGeneration: crypto.randomUUID(),
        deleted: false,
      });
      await this.input.library.registerDeck(control);
      const registered = (await this.input.library.listDeckEntries(true)).find(
        (entry) => entry.control.deckId === control.deckId,
      );
      if (!registered)
        throw new Error("Registered cloud deck is missing from the catalog");
    ...
    type Prepared = {
      candidate: CloudDeckControl;
      cloudSerial: number;
      payloadNames: string[];
      activation: CloudCuratedDeckActivation | null;
      revisions: CloudDeckRevision[];
      reviews: CloudReviewEvent[];
      skip: boolean;
      error?: unknown;
    };
    ...
    for (const [index, entry] of catalog.entries()) {
      ...
      try {
        ...
        const sameDeckGeneration =
          state?.control.deckGeneration === ledger.control.deckGeneration;
        const sameProgressGeneration =
          state?.control.progressGeneration ===
          ledger.control.progressGeneration;
        let activation: CloudCuratedDeckActivation | null = sameDeckGeneration
          ? (state?.curated ?? null)
          : null;
        let revisions: CloudDeckRevision[] = sameDeckGeneration
          ? [...(state?.knownRevisions ?? [])]
          : [];
        let payloadNames = sameDeckGeneration
          ? [...(state?.seenPayloadNames ?? [])]
          : [];
        const skip = Boolean(
          state?.lastResult &&
          state.cloudSerial === entry.serial &&
          !dirtyDecks.has(candidate.deckId),
        );
        const store = this.input.library.deckStore(ledger.control);
        if (ledger.deletion || ledger.control.deleted) {
          prepared.push({
            candidate,
            cloudSerial: entry.serial,
            payloadNames,
            activation,
            revisions: [],
            reviews: [],
        ...
        const localActivation = this.activation(
          ledger.control,
          localContent.get(candidate.deckId) ?? null,
        );
        if (
          localActivation &&
          (!activation || !same(activation, localActivation))
        )
          await this.publishActivation(store, localActivation);
        const reviews: CloudReviewEvent[] = [];
        if (!skip) {
          const names = await this.input.library.listPayloadNames(
            ledger.control,
          );
          const seen = new Set(
            payloadNames.filter(
              (name) => sameProgressGeneration || !name.startsWith("review."),
            ),
          );
          const changed = names.filter((name) => !seen.has(name));
          payloadNames = names;
          if (
            names.includes("activation.v1") &&
            (!activation || changed.includes("activation.v1"))
          ) {
        ...
        const header =
          cloudDeckRevisionHeads(revisions)[0]?.header ??
          revisions.find((revision) => revision.header)?.header;
        this.input.onDeck?.({
          deckId: candidate.deckId,
          title: fallback?.deck.title ?? header?.title ?? candidate.deckId,
          removed: state?.removed ?? false,
          status: "pending",
          revisions: cloudDeckRevisionHeads(revisions).map(
            (revision) => revision.revisionId,
          ),
          cardCount: fallback?.cards.length ?? header?.cardCount ?? 0,
          parentDeckId:
            fallback?.deck.parentDeckId ??
            activation?.parentDeckId ??
            header?.parentDeckId ??
            null,
          curated: Boolean(activation),
          localAvailable: Boolean(fallback),
        });
        prepared.push({
          candidate,
          cloudSerial: entry.serial,
    ...
    const ordered = prioritizeCloudDecks(
      prepared,
      (item) => item.reviews.length > 0,
      (item) => item.candidate.deckId,
      (item) =>
        item.activation?.parentDeckId ??
        localContent.get(item.candidate.deckId)?.deck.parentDeckId ??
        item.revisions.find((revision) => revision.header)?.header
          ?.parentDeckId ??
        null,
    );
    ...
    for (const [orderedIndex, preparedDeck] of ordered.entries()) {
      ...
      this.deckTitle =
        state?.base?.deck.title ??
        cloudContentFromSnapshot(initial, candidate.deckId)?.deck.title;
      this.progress("catalog");
      ...
      try {
        ...
        if (control.deleted) {
          ...
          results.push(
            this.emit({
              deckId: control.deckId,
              title: state?.base?.deck.title ?? control.deckId,
              removed: true,
              status: "deleted",
              revisions: [],
              cardCount: 0,
              parentDeckId:
                state?.base?.deck.parentDeckId ??
                state?.curated?.parentDeckId ??
                null,
        ...
        if (state && state.control.deckGeneration !== control.deckGeneration)
          throw new Error("Deck generation changed");
        if (
          state &&
          state.control.progressGeneration !== control.progressGeneration
        ) {
          await this.input.blockWrites();
        ...
        state ??= {
          ...
          revisionId: null,
          removed:
            this.input.downloadRemoteDecks === false &&
            !localContent.has(control.deckId),
          deleted: false,
          pending: null,
        };
        if (state.curated && !preparedDeck.activation)
          throw new Error(
            "Curated activation disappeared; preserve local data",
          );
        if (preparedDeck.activation) state.curated = preparedDeck.activation;
        if (
          state.removed &&
          preparedDeck.activation &&
          localContent.has(control.deckId)
        ) {
          // Reinstalling a curated catalog entry is an explicit reactivation.
          // Its cloud reviews must be read again before projection.
          state.removed = false;
          state.seenPayloadNames = state.seenPayloadNames?.filter(
            (name) => !name.startsWith("review."),
          );
        ...
        if (state.removed) {
          const pendingReviews = new Map(
            [...(state.pendingReviews ?? []), ...preparedDeck.reviews].map(
              (review) => [review.review.reviewId, review],
            ),
          );
          const result: CloudDeckSyncResult = {
            deckId: control.deckId,
            title:
              state.base?.deck.title ??
              state.curated?.sourceTemplateKey ??
              control.deckId,
            removed: true,
            status: "synced",
            revisions: [],
            cardCount: state.base?.cards.length ?? 0,
            parentDeckId:
              state.base?.deck.parentDeckId ??
              state.curated?.parentDeckId ??
              null,
          ...
          Object.assign(state, {
            cloudSerial: preparedDeck.cloudSerial,
            seenPayloadNames: preparedDeck.payloadNames,
            knownRevisions: revisions,
            pendingReviews: [...pendingReviews.values()],
            lastCheckedAt: (this.input.now?.() ?? new Date()).toISOString(),
            lastResult: result,
          });
          await this.save(state);
        ...
        if (preparedDeck.activation) {
          if (!this.input.installCuratedDeck)
            throw new Error("Curated catalog installer is unavailable");
          ...
          if (
            !local ||
            local.deck.sourceTemplateKey !==
              preparedDeck.activation.sourceTemplateKey
          )
            throw new Error(
              "Verified curated activation did not install the expected deck",
            );
          ...
          const localReviews = reviewsFromSnapshot(snapshot, control).filter(
            (event) => pendingReviewIds.has(event.review.reviewId),
          );
          ...
          const result: CloudDeckSyncResult = {
            ...
            parentDeckId: local.deck.parentDeckId ?? null,
            curated: true,
            localAvailable: true,
          };
          Object.assign(state, {
        ...
        const pendingReviews = new Set(
          snapshot.payload.mutationJournal
            .filter(
              (m) => m.entityType === "REVIEW" && outboxIds.has(m.mutationId),
            )
            .map((m) => m.entityId),
        );
        const localReviews = reviewsFromSnapshot(snapshot, control).filter(
          (event) => !state?.base || pendingReviews.has(event.review.reviewId),
        );
        this.progress("reviews", 0, localReviews.length);
        for (const [index, review] of localReviews.entries()) {
          await publishCloudReview(store, review, this.clock());
        ...
        if (state.pending) {
          const published = await this.publish(
            ...
            state.pending.revision.parentRevisionIds,
            preparedDeck.payloadNames,
          );
          if (
            pendingRevision &&
            !revisions.some(
              (revision) => revision.revisionId === pendingRevision.revisionId,
            )
          )
            revisions.push(pendingRevision);
          if (
            !revisions.some(
              (revision) => revision.revisionId === published.revisionId,
            )
          )
        ...
        for (const revision of revisions) {
          if (
            revision.deckId !== control.deckId ||
            revision.deckGeneration !== control.deckGeneration ||
            revision.libraryId !== control.libraryId ||
            revision.libraryGeneration !== control.libraryGeneration
          )
            throw new Error("Revision scope mismatch");
        }
        const heads = cloudDeckRevisionHeads(revisions);
        const choice = requestedResolutions.get(control.deckId);
        ...
        const chosen =
          choice && choice !== "local"
            ? heads.find((h) => h.revisionId === choice)
            : heads[0];
        if (choice && choice !== "local" && !chosen)
          throw new Error("Selected conflict revision changed");
        let content = local;
        ...
        if (chosen && choice !== "local") {
          ...
          if (state.revisionId === chosen.revisionId && state.base && local)
            remoteContent = state.base;
          else {
          ...
          content = choice
            ? remoteContent
            : mergeCloudContents(state.base, local, remoteContent);
        }
        if (!content)
          throw new Error("Deck has no complete content revision yet");
        ...
        if (choice || !chosen || !same(content, remoteContent)) {
          ...
          state.base = content;
          state.revisionId = chosen.revisionId;
          await this.save(state);
        }
        ...
        if (
          ...
          throw error;
        const fallback =
          state?.base ?? cloudContentFromSnapshot(initial, candidate.deckId);
        ...
        results.push(
          this.emit({
            ...
            removed: state?.removed ?? false,
            status:
              error instanceof CloudContentConflict ? "conflict" : "error",
            revisions: cloudDeckRevisionHeads(revisions).map(
              (r) => r.revisionId,
            ),
            problem: cloudTransferProblem(error),
            cardCount: fallback?.cards.length ?? header?.cardCount ?? 0,
            parentDeckId:
              fallback?.deck.parentDeckId ??
              state?.curated?.parentDeckId ??
    ...
    for (const m of snapshot.payload.mutationJournal) {
      if (
        ...
        (m.payload &&
          ...
          "deckId" in m.payload &&
          m.payload.deckId === deckId)
      ) {
        if (kind === "progress" && !["CARD", "REVIEW"].includes(m.entityType))
          continue;
        owned.add(m.entityId);
      }
    }
    const replacements: LocalMutationInput[] =
      kind === "progress"
        ? snapshot.payload.entities.flatMap((e) => {
            const m = e.winningMutation;
            if (
              !owned.has(m.entityId) ||
              m.entityType !== "CARD" ||
              m.operation === "DELETE"
            )
              return [];
        ...
        : [];
    this.input.checkActive?.();
    await this.input.authority.eraseCloudEntities(
      [...owned],
      replacements,
      snapshot.payload.replicaWatermarks,
    );
  }
  async executeCommand(command: {
    deckId: string;
    operationId: string;
    kind: "deck" | "progress" | "remove";
    ...
    if (command.kind === "remove") {
      ...
      if (state.curated) {
        ...
        if (
          !confirmed ||
          !same(
            this.parseActivation(confirmed.value, state.control),
            state.curated,
          )
        )
          throw new Error(
            "Curated activation is missing; preserve the local download",
          );
      } else {
        if (!state.revisionId || state.pending)
          throw new Error(
            "Deck upload must finish before removing its download",
          );
        const confirmed = await store.read(`revision.${state.revisionId}`);
        if (
          !confirmed ||
          cloudDeckRevisionSchema.parse(confirmed.value).revisionId !==
            state.revisionId
        )
          throw new Error(
            "Cloud revision is missing; preserve the local download",
          );
      }
      state.removed = true;
      state.cloudSerial = undefined;
    ...
    await this.input.library.beginDeletion(
      state.control,
      command.operationId,
  ...
  async discardLocalOnly(deckIds: readonly string[]) {
    const requested = new Set(deckIds);
    if (!requested.size) throw new Error("No decks selected");
    await this.input.assertAccount();
    const linked = (await this.input.library.listDeckEntries(true)).filter(
      (entry) => requested.has(entry.control.deckId) && !entry.control.deleted,
    );
    if (linked.length)
      throw new Error(
        "A cloud copy exists; synchronize before choosing the deletion scope",
      );
    for (const deckId of requested) await this.eraseLocal(deckId, "remove");
  }
}
