...
export function cloudContentFromSnapshot(snapshot: Snapshot, deckId: string): CloudDeckContent | null {
  ...
  return parseCloudDeckContent({ deckId, deck: localDeckPayloadSchema.parse(deck.payload),
    ...
    }).sort((left, right) => left.card.state.due.localeCompare(right.card.state.due) ||
      left.card.position - right.card.position || left.cardId.localeCompare(right.cardId))
...
export class CloudLibraryRuntime {
  ...
  private key(deckId: string) { return JSON.stringify(["deck-runtime-v3", this.input.environment,
  ...
  private async uploadPending(store: CloudRecordStore, pending: NonNullable<CloudDeckSyncState["pending"]>,
    ...
    const sources = [source(pending.revision.content), ...pending.media.map((asset) => source(asset.manifest))];
  ...
  async synchronize(resolve?: CloudConflictResolution | readonly CloudConflictResolution[]): Promise<CloudDeckSyncResult[]> {
    const requestedResolutions = new Map<string, string>();
    for (const resolution of resolve ? (Array.isArray(resolve) ? resolve : [resolve]) : []) {
      const previous = requestedResolutions.get(resolution.deckId);
      if (previous && previous !== resolution.revisionId) throw new Error("Conflicting resolutions for one deck");
    ...
    for (const [orderedIndex, preparedDeck] of ordered.entries()) {
      ...
      try {
        ...
        state ??= {control, base: null, revisionId: null,
          removed: this.input.downloadRemoteDecks === false && !localContent.has(control.deckId),
          deleted: false, pending: null};
