...
const binding = (): CloudLibraryBinding => ({
...
const scope = () => ({
  ...
  manifest: {
    sha256: "a".repeat(64),
    byteSize: 3,
    chunks: [{ index: 0, sha256: "b".repeat(64), byteSize: 3 }],
  },
});
...
function nativeFixture() {
  ...
  const sqlite = {
    ...
    query: vi.fn(async (input: { values: string[] }) => {
      ...
      return {
        values: [
          { ios_columns: ["entry_value"] },
          ...(value === undefined ? [] : [{ entry_value: value }]),
        ],
      };
    }),
    run: vi.fn(async (input: { values: string[] }) => {
      working!.set(input.values[0]!, input.values[1]!);
      return {};
    }),
  };
  return {
    sqlite: sqlite as unknown as NonNullable<
      Parameters<typeof createNativeCloudKeyValue>[0]
    >,
    rollback: sqlite.rollbackTransaction,
...
describe("durable cloud platform storage", () => {
  it("reopens browser staging without losing interrupted download chunks", async () => {
    ...
    const first = createCloudAssetStaging({
      ...scope(),
      values: createBrowserCloudKeyValue(name),
    });
    await first.writeChunk(0, new Uint8Array([1, 2, 3]));
    const restarted = createCloudAssetStaging({
      ...scope(),
      values: createBrowserCloudKeyValue(name),
    });
  ...
  it("isolates accounts, environments, libraries and generations", async () => {
    ...
    await createCloudAssetStaging({ ...scope(), values }).writeChunk(
      0,
  ...
  it("rejects bad chunk positions and sizes before writing", async () => {
    const staging = createCloudAssetStaging({
      ...scope(),
      values: createBrowserCloudKeyValue(crypto.randomUUID()),
    });
    await expect(staging.writeChunk(1, new Uint8Array(3))).rejects.toThrow(
      /index/,
    );
    await expect(staging.writeChunk(0, new Uint8Array(2))).rejects.toThrow(
      /size/,
    );
  ...
  it("serializes browser read-modify-write operations across adapter instances", async () => {
    const name = crypto.randomUUID();
    const a = createBrowserCloudKeyValue(name);
    const b = createBrowserCloudKeyValue(name);
    await Promise.all(
      Array.from({ length: 10 }, (_, index) =>
        (index % 2 ? a : b).update("count", (current) =>
  ...
  it("rolls back browser binding conflicts without changing the durable account", async () => {
    const name = crypto.randomUUID();
    const bindings = createNativeCloudLibraryBindings(
      createBrowserCloudKeyValue(name),
    );
    const pending = await bindings.reserve(binding());
    await bindings.confirm(pending, pending.root);
    await expect(
      bindings.reserve({ ...binding(), account: "account-b" }),
    ).rejects.toThrow("ACCOUNT_MISMATCH");
    const restarted = createNativeCloudLibraryBindings(
      createBrowserCloudKeyValue(name),
    );
    expect(await restarted.read("development")).toMatchObject({
      account: "account-a",
      phase: "bound",
    });
  ...
  it("keeps SQLite binding and staging across adapter recreation", async () => {
    ...
    expect(
      await createNativeCloudLibraryBindings(restarted).read("development"),
  ...
  it("never reports SQLite success before commit and retries after interruption", async () => {
    const fixture = nativeFixture();
    const values = createNativeCloudKeyValue(
      fixture.sqlite,
      crypto.randomUUID(),
    );
    await values.update("key", () => "old");
    fixture.interrupt(true);
    await expect(values.update("key", () => "new")).rejects.toThrow(
      "interrupted commit",
    );
    expect(fixture.rollback).toHaveBeenCalledOnce();
    expect(await values.read("key")).toBe("old");
    fixture.interrupt(false);
    await values.update("key", () => "new");
  ...
  it("rejects native account/generation replacement without erasing the old binding", async () => {
    ...
    await bindings.confirm(pending, pending.root);
    await expect(
      bindings.confirm(pending, { ...pending.root, libraryGeneration: id(99) }),
  ...
  it("durably adopts a validated replacement development generation", async () => {
    const fixture = nativeFixture();
    const values = createNativeCloudKeyValue(
      fixture.sqlite,
      crypto.randomUUID(),
    ...
    const pending = await bindings.reserve(binding());
    await bindings.confirm(pending, pending.root);
    const bound = (await bindings.read("development"))!;
    const replacement = { ...bound.root, libraryGeneration: id(99) };
    await bindings.adoptDevelopment!(bound, replacement);
    expect((await bindings.read("development"))?.root).toEqual(replacement);
  });
});
