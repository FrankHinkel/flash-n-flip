...
export function createNativeCloudKeyValue(
  ...
  const read = async (key: string): Promise<string | null> => {
    const result = await sqlite.query({
      database,
      statement:
        "SELECT entry_value FROM cloud_durable_values WHERE entry_key = ?;",
      values: [key],
    });
    return (
      nativeSqliteRows<{ entry_value: string }>(result.values)[0]
        ?.entry_value ?? null
    );
  };
  return {
...
export function createBrowserCloudKeyValue(
  database = databaseName,
): CloudDurableKeyValue {
  const open = (): Promise<IDBDatabase> =>
    new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(
          request.error ?? new Error("Cloud staging database unavailable"),
        );
      request.onblocked = () =>
        reject(new Error("Close the other app tab to open cloud staging"));
      request.onsuccess = () => {
      ...
      return await new Promise<string | null>((resolve, reject) => {
        ...
        tx.onabort = () =>
          reject(
            failure ??
              tx.error ??
              new Error("Cloud staging transaction aborted"),
          );
        tx.onerror = () => {
          /* onabort reports the durable transaction outcome. */
        };
  ...
  return {
    read: (key) => access(key),
    update: async (key, change) => (await access(key, change))!,
  };
}
...
export function createNativeCloudLibraryBindings(
  ...
  const parse = (value: string | null) =>
    value === null ? null : cloudLibraryBindingSchema.parse(JSON.parse(value));
  return {
    async read(environment) {
      const binding = parse(await values.read(`binding.${environment}`));
      if (binding && binding.environment !== environment)
        throw new Error("Cloud binding environment mismatch");
      return binding;
    },
    async reserve(candidate) {
      return cloudLibraryBindingSchema.parse(
        JSON.parse(
          await values.update(`binding.${candidate.environment}`, (value) =>
            JSON.stringify(reserveCloudLibraryBinding(parse(value), candidate)),
          ),
        ),
      );
    },
    async confirm(expected, root) {
      await values.update(`binding.${expected.environment}`, (value) =>
        JSON.stringify(
          confirmCloudLibraryBinding(parse(value), expected, root),
        ),
      );
    },
    async adoptDevelopment(expected, root) {
      await values.update(`binding.${expected.environment}`, (value) =>
        JSON.stringify(
          adoptDevelopmentCloudLibraryBinding(parse(value), expected, root),
        ),
      );
    },
  };
...
export function createCloudAssetStaging(input: {
  ...
  if (
    !input.account ||
    input.account.length > 1024 ||
    !["development", "production"].includes(input.environment)
  )
    throw new Error("Invalid cloud staging account");
  ...
  const key = (index: number): string => {
    if (!Number.isSafeInteger(index) || !manifest.chunks[index])
      throw new Error("Invalid cloud chunk index");
    return JSON.stringify([
      "chunk",
      input.environment,
      input.account,
      identity.libraryId,
      identity.libraryGeneration,
      manifest.sha256,
  ...
  return {
    async readChunk(index) {
      ...
      try {
        ...
        return null;
      } // The transfer re-fetches corrupt staging chunks.
    },
    async writeChunk(index, bytes) {
      const entryKey = key(index);
      if (bytes.byteLength !== manifest.chunks[index]!.byteSize)
        throw new Error("Cloud staging chunk size mismatch");
      let binary = "";
