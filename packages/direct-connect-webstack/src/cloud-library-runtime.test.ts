...
const deck = localDeckPayloadSchema.parse({
  title: "Two devices",
  description: "",
  language: "de",
  createdAt: time,
...
class Cloud implements CloudAtomicStore {
  ...
  async atomic(ops: readonly CloudAtomicOperation[]) {
    ...
    for (const op of ops) {
      ...
      if ((next.get(op.name)?.changeTag ?? null) !== op.expectedTag)
        throw new CloudLibraryError("WRITE_CONFLICT", "conflict");
      next.set(op.name, {
        value: structuredClone(op.value),
        changeTag: String(++this.serial),
      });
    }
    this.records = next;
    if (
      this.loseNextReviewReply &&
      ops.some(
        (op) =>
          op.kind === "save" &&
          op.value &&
      ...
      this.loseNextReviewReply = false;
      throw new Error("Lost reply after durable cloud commit");
    }
  }
}
class Device {
  ...
  constructor(
    readonly library: AtomicCloudLibrary,
    readonly installCuratedDeck?: (
  ...
  open() {
    return new CloudLibraryRuntime({
      ...
      authority: this.repository.cloudAuthority,
      media: createLocalMediaStorage(),
      values: createBrowserCloudKeyValue(),
      assertAccount: async () => {},
      blockWrites: async () => {},
  ...
  async run<T>(operation: () => Promise<T>) {
    vi.stubGlobal("indexedDB", this.db);
    return operation();
  }
  sync() {
    return this.run(() => this.runtime.synchronize());
  }
  async seed(deckPayload = deck, cardPayload = card) {
    await this.run(() =>
      this.repository.authority.commitLocalMutations([
        {
          entityId: id(3),
          entityType: "DECK",
          operation: "UPSERT",
  ...
  review(reviewId: number, at: string, rating: "GOOD" | "HARD" = "GOOD") {
    return this.run(() =>
      this.repository.reviewCard(id(4), rating, new Date(at), id(reviewId)),
    );
  }
  restart() {
    this.repository = new LocalAppRepository(id(90));
    this.runtime = this.open();
  }
}
async function fixture() {
  ...
  const a = new Device(library),
    b = new Device(library);
  await a.seed();
...
describe("complete cloud runtime with independent IndexedDB devices", () => {
  it("activates curated hierarchies parent-first and transfers reviews without deck content", async () => {
    ...
    const library = new AtomicCloudLibrary(cloud, identity, cloudCodec.hash);
    await library.initialize();
    const parent = localDeckPayloadSchema.parse({
      ...deck,
      title: "Africa",
    ...
    const child = localDeckPayloadSchema.parse({
      ...deck,
      parentDeckId: id(3),
      title: "Ghana",
      sourceTemplateKey: "geography:ghana:v1",
    ...
    await a.run(() =>
      a.repository.authority.commitLocalMutations([
        {
          entityId: id(5),
          entityType: "DECK",
          operation: "UPSERT",
    ...
    expect((await a.sync()).every((result) => result.status === "synced")).toBe(
      true,
    );
    ...
    for (const control of controls) {
      ...
      expect(
        names.some(
          (name) => name.startsWith("revision.") || name.startsWith("asset."),
        ),
      ).toBe(false);
    }
    expect(
      (await library.listPayloadNames(controls[1]!)).some(
        (name) => name.startsWith("review.") && name.endsWith(id(20)),
      ),
    ).toBe(true);

    const installed: string[] = [];
    let b!: Device;
    b = new Device(library, async (activation) =>
      b.run(async () => {
        installed.push(activation.sourceTemplateKey);
        if (activation.deckId === id(3)) {
          await b.repository.authority.commitLocalMutation({
            entityId: id(3),
            entityType: "DECK",
            operation: "UPSERT",
            baseVersion: null,
    ...
    expect((await b.sync()).every((result) => result.status === "synced")).toBe(
      true,
    );
    expect(installed).toEqual(["geography:africa:v1", "geography:ghana:v1"]);
    await b.run(async () => {
      expect(
        (await b.repository.listDecks()).map((entry) => [
          entry.id,
          entry.payload.parentDeckId,
        ]),
      ).toEqual([
        [id(3), null],
        [id(5), id(3)],
      ...
      expect(
        (await b.repository.getCard(id(4)))?.payload.state.lastReview,
      ).toBe("2026-09-06T10:10:00.000Z");
    });
  });
  ...
  it("transfers deck, card and verified media, reopens and repeats without duplicates", async () => {
    ...
    b.restart();
    await b.run(async () => {
      expect((await b.repository.listDecks())[0]?.payload.title).toBe(
        deck.title,
      );
      expect((await b.repository.getCard(id(4)))?.payload.front).toEqual(
        card.front,
      );
      expect((await b.repository.getMedia(id(8)))?.bytes).toEqual(
    ...
    expect((await b.sync())[0]?.status).toBe("synced");
    expect(
      cloud.reads.filter((name) => name.startsWith("payload.")),
    ).toHaveLength(0);
  ...
  it("latest actual review wins even when the older offline review uploads last", async () => {
    ...
    for (const device of [a, b])
      await device.run(async () => {
        expect(
          (await device.repository.getCard(id(4)))?.payload.state.lastReview,
        ).toBe("2026-09-06T10:20:00.000Z");
  ...
  it("keeps offline outbox and resumes a review committed before a lost reply", async () => {
    ...
    expect(await a.run(() => a.repository.authority.countOutbox())).toBe(
      pending,
    );
    cloud.offline = false;
    cloud.loseNextReviewReply = true;
  ...
  it("retains cloud progress when removing and restoring a local download", async () => {
    ...
    expect(await a.run(() => a.repository.listReviews())).toHaveLength(0);
    a.restart();
    expect((await a.sync())[0]?.removed).toBe(true);
    await a.run(() => a.runtime.restoreDownload(id(3)));
    expect((await a.sync())[0]?.status).toBe("synced");
    expect(
      (await a.run(() => a.repository.getCard(id(4))))?.payload.state.reps,
    ).toBe(1);
  });
  it("discards a confirmed local-only deck without creating cloud state", async () => {
    const { a, library } = await fixture();
    await a.run(() => a.runtime.discardLocalOnly([id(3)]));
    expect(await a.run(() => a.repository.listDecks())).toHaveLength(0);
    expect(await library.listDecks()).toHaveLength(0);
  });
  it("preserves curated cloud progress across local deactivation", async () => {
    const cloud = new Cloud();
    const library = new AtomicCloudLibrary(cloud, identity, cloudCodec.hash);
    await library.initialize();
    const curated = localDeckPayloadSchema.parse({
      ...deck,
      sourceTemplateKey: "geography:africa:v1",
      sourceContentSha256: "a".repeat(64),
      sourcePublishedAt: time,
    ...
    let device!: Device;
    device = new Device(library, async () => {
      await device.run(async () => {
        if (
          (await device.repository.listDecks()).some(
            (entry) => entry.id === id(3),
          )
        )
          return;
        await device.repository.authority.commitLocalMutations([
          {
            entityId: id(3),
            entityType: "DECK",
            operation: "UPSERT",
    ...
    await device.seed(curated, card);
    await device.review(20, "2026-09-06T10:10:00.000Z");
    await device.sync();
    await device.run(() =>
      device.runtime.executeCommand({
        deckId: id(3),
        operationId: id(30),
        kind: "remove",
        nextGeneration: id(31),
    ...
    expect(await device.run(() => device.repository.listDecks())).toHaveLength(
      0,
    );
    expect(
      await device.run(() => device.repository.listReviews()),
    ).toHaveLength(0);
    await device.run(() => device.runtime.restoreDownload(id(3)));
    expect((await device.sync())[0]?.status).toBe("synced");
    expect(
      (await device.run(() => device.repository.getCard(id(4))))?.payload.state
        .reps,
    ).toBe(1);
  });
  it("physically resets cloud reviews and discards stale offline progress before publishing", async () => {
    ...
    expect(
      (await b.run(() => b.repository.getCard(id(4))))?.payload.state.reps,
    ).toBe(0);
    const current = (await library.listDecks())[0]!;
    expect(
      (await library.listPayloadNames(current)).some((name) =>
        name.startsWith("review."),
      ),
    ).toBe(false);
    const { deleted: _deleted, ...scope } = old;
    const event = cloudReviewEventSchema.parse({
      ...scope,
      review: {
        reviewId: id(40),
        deckId: id(3),
        ...
        parameters: [1],
        before: initialState,
        after: { ...initialState, reps: 1, lastReview: time },
      },
    });
    await expect(
      publishCloudReview(library.deckStore(old), event, {
  ...
  it("never repeats a completed local reset over newly recorded reviews", async () => {
    ...
    await a.run(() => a.runtime.executeCommand(second));
    expect(
      (await a.run(() => a.repository.listReviews())).map(
        (entry) => entry.payload.reviewId,
      ),
    ).toEqual([id(23)]);
  });
  ...
  it("requires explicit resolution for concurrent content edits", async () => {
    ...
    for (const [device, title] of [
      ...
      await device.run(async () => {
        ...
        await device.repository.authority.commitLocalMutation({
          entityId: id(3),
          entityType: "DECK",
          operation: "UPSERT",
          baseVersion: current.version,
          payload: { ...current.payload, title },
        });
      });
    expect((await a.sync())[0]?.status).toBe("synced");
    ...
    expect(
      (
        await b.run(() =>
          b.runtime.synchronize([{ deckId: id(3), revisionId: "local" }]),
        )
      )[0]?.status,
    ).toBe("synced");
    expect(() =>
      mergeCloudContents(
        ...
        {
          deckId: id(3),
          deck: { ...deck, title: "Different" },
          cards: [],
          media: [],
        },
      ),
    ).toThrow(/Concurrent cloud content/);
  });
  it("fences peers and legacy resets durably, including after pause/restart", async () => {
    const { a } = await fixture();
    await a.run(async () => {
      await updateCloudPolicy(() => ({
        account: "account-a",
        environment: "development",
        enabled: false,
        blocked: false,
      ...
      await expect(
        a.repository.resetDeckProgress(new Set([id(3)])),
      ).rejects.toThrow(/iCloud/);
      expect((await readCloudPolicy())?.enabled).toBe(false);
      await updateCloudPolicy((policy) => ({ ...policy!, blocked: true }));
      await expect(
        a.repository.reviewCard(id(4), "GOOD", new Date(time), id(22)),
      ).rejects.toThrow(/Loeschauftrag/);
    });
  });
});
