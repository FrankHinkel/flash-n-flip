...
export const normalizePreservedAnkiLayouts = <
...
>(
  ...
  for (const card of parsed.decks.flatMap((deck) => deck.cards)) {
    ...
    const questionMaskField = findField(/(?:question|frage).*mask/i);
    const answerMaskField = findField(/(?:answer|antwort).*mask/i);
    const headerField = findField(/^(?:header|kopf|überschrift)$/i);
    if (!imageField || !questionMaskField || !answerMaskField) continue;
    const baseSourceName = firstImportedImageName(
      card.sourceFields?.[imageField],
    );
    const questionOverlay = firstImportedImageName(
      card.sourceFields?.[questionMaskField],
    );
    const answerOverlay = firstImportedImageName(
      card.sourceFields?.[answerMaskField],
    );
    if (!baseSourceName || !questionOverlay || !answerOverlay) continue;
    const header = headerField
      ? (card.sourceFieldText?.[headerField]?.trim() ?? "")
      : "";
    const leading: AnkiContentBlock[] = header
    ...
    card.front = {
      blocks: [
        ...leading,
        {
          type: "imageOverlay",
          baseSourceName,
          overlaySourceName: questionOverlay,
          alt: header || "Image Occlusion",
          decorative: false,
        },
      ],
    };
    card.back = {
      blocks: [
        ...leading,
        {
          type: "imageOverlay",
          baseSourceName,
          overlaySourceName: answerOverlay,
          alt: header || "Image Occlusion",
          decorative: false,
        },
      ],
...
const defaultMapping = (
  ...
  const mediaFields = new Set(
    noteType.fields.filter(
      (field) =>
        cards.some(
          (card) => mediaKinds(blocksForField(card, field)).length > 0,
        ) && !hasText(field),
    ),
  );
...
export const applyAnkiFieldMappings = <TData extends Uint8Array = Uint8Array>(
  parsed: ParsedAnkiPackage<TData>,
