...
function local() {
  ...
  const repository: CloudLibraryBindingRepository = {
    ...
    reserve: async (candidate) =>
      (binding = reserveCloudLibraryBinding(binding, candidate)),
    confirm: async (expected, remote) => {
      binding = confirmCloudLibraryBinding(binding, expected, remote);
    },
  };
  return repository;
}
function remote() {
  ...
  const store: CloudRecordStore = {
    ...
    compareAndSwap: vi.fn(async (_name, tag, candidate) => {
      if (value || tag !== null)
        throw new CloudLibraryError("WRITE_CONFLICT", "Already exists");
      value = candidate;
    }),
  };
  return {
...
describe("durable private library bootstrap", () => {
  it("adopts a confirmed replacement generation only in development", () => {
    const bound = {
      account: "account-a",
      environment: "development" as const,
    ...
    expect(
      adoptDevelopmentCloudLibraryBinding(bound, bound, root("2")),
    ).toEqual({ ...bound, root: root("2") });
    expect(() =>
      adoptDevelopmentCloudLibraryBinding(
  ...
  it("reserves locally before cloud access and confirms only after readback", async () => {
    ...
    options.storeForAccount.mockImplementation(() => {
      expect(options.bindings.read("development")).resolves.toMatchObject({
        phase: "pending",
      });
      return db.store;
    });
    const identity = await connectCloudLibrary(options);
    expect(db.store.compareAndSwap).toHaveBeenCalledWith(
      cloudLibraryRootRecordName,
      null,
      identity,
    );
    expect(await options.bindings.read("development")).toMatchObject({
      phase: "bound",
      root: identity,
    });
  });
  ...
  it("adopts an existing root without writing over it", async () => {
    const db = remote();
    db.replace(root("2"));
    expect(await connectCloudLibrary(input(local(), db.store))).toEqual(
      root("2"),
    );
  ...
  it("converges two independently initialized devices on one root", async () => {
    const db = remote();
    const a = input(local(), db.store),
      b = input(local(), db.store);
    b.randomUUID = () => root("2").libraryId;
    const [first, second] = await Promise.all([
      connectCloudLibrary(a),
      connectCloudLibrary(b),
    ]);
    expect(first).toEqual(second);
    expect(await b.bindings.read("development")).toMatchObject({
      phase: "bound",
      root: first,
    });
  });
  ...
  it("recovers when a successful create response is lost", async () => {
    ...
    db.store.compareAndSwap = vi.fn(
      async (...args: Parameters<CloudRecordStore["compareAndSwap"]>) => {
        await write(...args);
        throw new Error("connection lost after write");
      },
    );
    await expect(connectCloudLibrary(options)).rejects.toThrow(
      "connection lost",
    );
  ...
  it("refuses a different account before any remote access", async () => {
    ...
    await expect(
      connectCloudLibrary({ ...options, account: "account-b" }),
    ).rejects.toMatchObject({ code: "ACCOUNT_MISMATCH" });
    expect(options.storeForAccount).not.toHaveBeenCalled();
  });

  it.each([root("2"), { ...root(), deleted: true }])(
    "refuses changed or deleted roots",
    async (changed) => {
      const db = remote();
      db.replace(root());
      const options = input(local(), db.store);
  ...
  it("does not treat an outage as an empty cloud", async () => {
    const db = remote();
    vi.mocked(db.store.read).mockRejectedValue(new Error("offline"));
    const options = input(local(), db.store);
  ...
  it("fails on unsupported remote schema without rewriting it", async () => {
    const db = remote();
    db.replace({ ...root(), protocolVersion: 2 });
    await expect(
      connectCloudLibrary(input(local(), db.store)),
    ).rejects.toThrow();
    expect(db.store.compareAndSwap).not.toHaveBeenCalled();
  });
  ...
  it("does not acknowledge local persistence failure as successful linking", async () => {
    ...
    bindings.confirm = async () => {
      throw new Error("disk full");
    };
    await expect(connectCloudLibrary(input(bindings))).rejects.toThrow(
      "disk full",
    );
    expect(await bindings.read("development")).toMatchObject({
      phase: "pending",
    });
  });
});
